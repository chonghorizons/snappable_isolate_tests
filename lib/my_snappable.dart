...
//    _layers = await compute<List<image.Image>, List<Uint8List>>(_encodeImages, _images);
    _layers = await specialTask(_images);
...
specialTask(List<image.Image> _images) async {
  ...
  switch (method) {
    case 1:
      {
        int numberOfBuckets = _images.length;
        List<Uint8List> outputList = List(numberOfBuckets);
        for (int i = 0; i < numberOfBuckets; i++) {
          print('processing ' + i.toString() + "|" + DateTime.now().toString());
          outputList[i] = Uint8List.fromList(image.encodePng(_images[i]));
        }
        print(DateTime.now().toString());
        return outputList;
      }
    case 2:
      {
        ...
        List<Future> futures =  List(numberOfBuckets);
        for (int i = 0; i < numberOfBuckets; i++) {
          print('processing ' + i.toString() + "|" + DateTime.now().toString());
          futures[i] =
            compute(
    ...
    default:
      {
        print('begin');
        var x;
        // _layers = specialTask(_images);
        x = await compute<List<image.Image>, List<Uint8List>>(
...
Uint8List _processImage(localimage) {
  var a=  Uint8List.fromList(image.encodePng(localimage));
  print(DateTime.now().toString());
  return a;
}
